/* Generates design-3 (portal style) subpages: album, jobs, location, contact.
   Reuses shared page content + the portal design CSS from the home file. */
const fs = require('fs');
...
const home = read('design-3-portal-style.html');

/* Extract CSS from home */
const css = home.match(/<style>([\s\S]*?)<\/style>/)[1];

/* Extract topbar (brand + nav + cta) and footer */
const topbarStart = home.indexOf('<header class="topbar">');
const topbarEnd = home.indexOf('</header>') + 9;
const topbar = home.slice(topbarStart, topbarEnd);
const footerStart = home.indexOf('<footer class="footer">');
...
function makeNav(active, mode) {
  ...
  NAV_KEYS.forEach(k => {
    ...
    items += `    <a href="${navHref(k, mode)}"${cls}${ext}>${NAV_LABELS[k]}</a>\n`;
  });
  return `<nav class="navbar">\n${items}  </nav>`;
}

const extraCss = `
/* subpages (portal style) */
.subpage-head{
  background:
  ...
  color:#fff;padding:64px 0 56px;text-align:center;margin-bottom:44px;position:relative;overflow:hidden;
}
.subpage-head::before{
...
.subpage-title{
  font-size:1.2rem;color:var(--primary-darker);margin:34px 0 16px;
  border-right:4px solid var(--primary);padding-right:14px;display:flex;align-items:center;gap:8px;
}
.subpage-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:20px}
.subpage-item{margin:0;border-radius:14px;overflow:hidden;border:1px solid var(--border);box-shadow:var(--shadow);transition:all .22s}
.subpage-item:hover{box-shadow:var(--shadow-lg);transform:translateY(-3px)}
.subpage-item img{width:100%;aspect-ratio:4/3;object-fit:cover;transition:.4s}
.subpage-item:hover img{transform:scale(1.05)}
.loc-wrap{display:grid;grid-template-columns:1.6fr 1fr;gap:26px;align-items:start}
.loc-map{aspect-ratio:4/3;border-radius:18px;overflow:hidden;box-shadow:var(--shadow-lg);border:4px solid rgba(37,99,235,.3)}
.loc-side{background:#fff;border:1px solid var(--border);border-radius:16px;padding:26px;box-shadow:var(--shadow);text-align:center}
.loc-qr{width:170px;margin:0 auto 12px;border-radius:10px}
.contact-wrap{display:grid;grid-template-columns:1.4fr 1fr;gap:26px;align-items:start}
.contact-form{background:#fff;border:1px solid var(--border);border-radius:16px;padding:28px;box-shadow:var(--shadow);display:grid;gap:14px}
.contact-form label{font-weight:700;color:var(--ink);display:grid;gap:6px;font-size:.95rem}
.contact-form input,.contact-form textarea{
  padding:12px 14px;border:1.5px solid var(--border);border-radius:12px;font-family:inherit;font-size:.95rem;
...
.contact-info{background:#fff;border:1px solid var(--border);border-radius:16px;padding:26px;box-shadow:var(--shadow)}
.contact-info h3{color:var(--primary-darker);margin-bottom:14px}
...
${topbar.replace(home.slice(home.indexOf('<nav class="navbar">'), home.indexOf('</nav>') + 6), makeNav(active, 'sub'))}
...
${footer
  .replace(home.slice(home.indexOf('<nav class="navbar">'), home.indexOf('</nav>') + 6), makeNav(active, 'sub'))
  .replace(/<div class="foot-qr">[\s\S]*?<\/div>\s*<\/div>/, '')
...
fs.mkdirSync(path.join(ROOT, 'design-3'), { recursive: true });
const homeNav = home.slice(home.indexOf('<nav class="navbar">'), home.indexOf('</nav>') + 6);
const homeFixed = home
  .replace(homeNav, makeNav('index', 'home'))
  .replace(/<div class="foot-qr">[\s\S]*?<\/div>\s*<\/div>/, '')
  .replace('</div>\n    </div>\n  </div>\n  <div class="bottom">', '</div>\n    </div>\n' + FOOTER_QR + '\n  </div>\n  <div class="bottom">');
write('design-3/index.html', homeFixed);
write('design-3/album.html', buildPage('ألبوم الصور', 'album', CONTENT.album));
write('design-3/jobs.html', buildPage('وظائف خالية', 'jobs', CONTENT.jobs));
write('design-3/contact.html', buildPage('تواصل معنا', 'contact', CONTENT.contact));

console.log('Generated design-3 (index/album/jobs/contact)');